...
import { llmResponse, llmCode } from "../model/llmResponse";

export async function handleAcceptAllChanges(
  acceptedResponse: llmResponse,
  codeArray: any
) {
  let openedFile = vscode.window.activeTextEditor?.document.uri.toString();

  for (let change of acceptedResponse.code) {
...
export async function handleDeclineAllChanges(
  declinedResponse: llmResponse,
  codeArray: any
) {
  let openedFile = vscode.window.activeTextEditor?.document.uri.toString();

  for (let change of declinedResponse.code) {
    if (change.hasPendingChanges) {
...
export async function handleRequestingHistory(
  providerInstance: webview.CodingBuddyViewProvider
) {
...
export async function handleChangeOpenedFile(
  value: any,
  providerInstance: webview.CodingBuddyViewProvider
) {
...
export async function handleChatDeletionRequested(
  providerInstance: webview.CodingBuddyViewProvider
) {
